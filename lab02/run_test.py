...
python run_test.py ../sols/main in/ out/ out/
...
def get_args():
    ...
    parser.add_argument(
        "gt_output_folder", help="Pasta com as saídas esperadas (corretas)"
    )
...
def run_tests(args):
    ...
    for file in input_files:
        ...
        print(
            f"[INFO] Executando teste para {file} | Saída salva em {test_output_file}"
        )

        # Escreve a saída da execução na pasta de teste
        with open(test_output_file, "w") as file:
            subprocess.call([args.main, f"{input_file}"], stdout=file)
        ...
        diff = difflib.unified_diff(test_content.split("\n"), gt_content.split("\n"))
        for line in diff:
            print(line)
